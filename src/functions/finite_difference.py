...
def diag_hamiltonian(V_flat, dr, hexagonal, order, conv_factor = None):
    if conv_factor is None:
        conv_factor = CONVERSION_FACTOR
    if hexagonal:
        L = hex_laplacian(len(potential), dr, order=order, conv_factor=conv_factor)
    else:
        L = laplacian(len(potential), dr, order=order, conv_factor=conv_factor)
