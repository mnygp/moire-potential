...
def strain(atoms: Atoms,
           atom_type: str,
           layer: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    if (layer not in ['top', 'bottom']):
        raise ValueError("Input not a valid layer type."
...
def interlayer_distance(atoms: Atoms,
                        TM: str) -> tuple[np.ndarray,
                                          np.ndarray,
                                          np.ndarray]:
    ...
    x, y, z = repeate_cells(bottom_atoms[:, 0],
                            bottom_atoms[:, 1],
                            bottom_atoms[:, 2],
                            range(-1, 2), vector1, vector2)
