...
for s, name in zip(structs, ["DFT", "MACE", "MatterSim"]):
    ...
    if name != "DFT":
        ...
        print(
            f"Average error in z dist for {name}: {z_error:.3f} +/- {z_error_std:.3f}"
        )
        print(
            f"Error range for {name}: {min(z_dft - z_dist) - max(z_dft - z_dist):.4f}"
        )
        print()
        ...
        print(
            f"Average error in Mo strain for {name}: {Mo_error:.5f} +/- {Mo_error_std:.5f}"
        )

        W_error = np.mean(W_strain_dft - W_strain)
        W_error_std = np.std(W_strain_dft - W_strain)
        print(
            f"Average error in W strain for {name}: {W_error:.5f} +/- {W_error_std:.5f}"
        )
...
ax.hist(mace_dif, bins=100, range=(min_val, max_val), alpha=0.8, label="MACE")
ax.hist(mattersim_dif, bins=100, range=(min_val, max_val), alpha=0.8, label="MatterSim")
...
for data, color, name in zip(
    [DFT_data, MAtterSim_data], ["C0", "C2"], ["DFT", "MatterSim"]
):
    z_dist = data[:, 1]
