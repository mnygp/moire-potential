...
strain_data = np.genfromtxt(
    "../gap-changes/band_edges_wide.csv", delimiter=",", skip_header=1
)
...
corrections = [-WSe2_lattice_corr, LUMO_correction - HOMO_correction, MoS2_lattice_corr]

print(corrections)

for i, lattice, name in zip(
    ...
    for j, shift in enumerate([0.0, 0.1, 0.2, 0.3]):
        ...
        ax2.plot(z, homo - ref_homo, "-o", color=f"C{2 * j}", label=f"{shift}-HOMO")
        ax2.plot(z, lumo - ref_lumo, "-*", color=f"C{2 * j + 1}", label=f"{shift}-LUMO")
    ...
    ax1.set(
        title=f"Energies with lattice constant equal to {name}",
        xlabel="z distance [Å]",
    ...
    ax2.set(
        title=f"Energies normalised with lattice constant equal to {name}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax2.legend()
    ...
    ax3.set(
        title=f"Band gap with lattice constant equal to {name}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax3.legend()
    ...
    ax4.set(
        title=f"Band gap normalised with lattice constant equal to {name}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax4.legend()
...
for i, shift in enumerate([0.0, 0.1, 0.2, 0.3]):
    ...
    for j, lattice, name in zip(
        ...
        ax7.plot(
            z, (lumo - homo) - max(lumo - homo), "-o", color=f"C{j}", label=f"{name}"
        )

        ax8.plot(
            z, (lumo - homo) + corrections[j], "-o", color=f"C{j}", label=f"{name}"
        )

    ax5.set(
        title=f"Band gap with shift {shift}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax5.legend()
    ...
    ax6.set(
        title=f"Band gap normalised with shift {shift}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax6.legend()
    ...
    ax7.set(
        title=f"Band gap normalised to 6.9Å with shift {shift}",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax7.legend()
    ...
    ax8.set(
        title=f"Band gap with strain correction from {name} lattice",
        xlabel="z distance [Å]",
        ylabel="Energy - Vacuum [eV]",
    )
    ax8.legend()
