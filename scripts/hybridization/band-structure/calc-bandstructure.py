...
a = 3.2515
data = np.loadtxt(f'../diagonal-shift/gap_shift_{a:.2f}_lcao.csv', delimiter=',', skiprows=1)
shift_arr = data[:, 0]
z_dist_arr = data[:, 1]
gap_arr = data[:, 2]
gap_arr_soc = data[:, 3]
...
for i in range(15, 20):
    struct = Trajectory(f'../diagonal-shift/traj_files/opt_{a:.2f}_{shift_arr[i]:.2f}.traj')[-1]
    ...
    # With SOC
    calc_load = GPAW(gpw_name).fixed_density(symmetry='off',
                                             kpts={'path': 'GMKG', 'npoints': 200})
    ...
    e_kn = np.array([calc_load.get_eigenvalues(kpt=k)[:]
