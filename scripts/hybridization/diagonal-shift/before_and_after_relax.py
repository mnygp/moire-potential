...
def get_gaps(struct):
    ...
    gap_scissors = calc_gap(struct, kpts=36, functional='PBE', mode='lcao', eigensolver=eigsolver)[0]

    gap_scissors_soc = alc_gap(struct, kpts=36, functional='PBE', mode='lcao', soc=True, eigensolver=eigsolver)[0]

    symbols = np.array(struct.get_chemical_symbols())
    z_dist = (struct[symbols == 'W'].positions[0][2] - struct[symbols == 'Mo'].positions[0][2])
...
for a in [3.2515]:  # np.linspace(3.184, 3.319, 7):
    ...
    for shift in shift_arr:
        struct = create_bilayer(6.3, lattice_length=a,
        ...
        (pre_relax_z_dist, pre_relax_gap, pre_relax_gap_soc, pre_relax_gap_scissors,
         pre_relax_gap_scissors_soc) = get_gaps(struct)
        ...
        (post_relax_z_dist, post_relax_gap, post_relax_gap_soc, post_relax_gap_scissors,
         post_relax_gap_scissors_soc) = get_gaps(struct)
    ...
    rows = zip(shift_arr, z_dist_arr_before, gap_arr_before, gap_soc_arr_before, gap_scissors_arr_before,
               gap_scissors_soc_arr_before)
    with open(f'before_and_after/gap_shift_{a:.2f}_lcao_before_relax.csv', mode='w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['shift', 'z_dist', 'gap', 'gap_soc', 'gap_scissors', 'gap_scissors_soc'])
        writer.writerows(rows)

    rows = zip(shift_arr, z_dist_arr_after, gap_arr_after, gap_soc_arr_after, gap_scissors_arr_after,
               gap_scissors_soc_arr_after)
    with open(f'before_and_after/gap_shift_{a:.2f}_lcao_after_relax.csv', mode='w', newline='') as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(['shift', 'z_dist', 'gap', 'gap_soc', 'gap_scissors', 'gap_scissors_soc'])
        writer.writerows(rows)