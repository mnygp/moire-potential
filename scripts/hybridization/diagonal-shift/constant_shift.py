...
for dist in dist_arr:
    struct = create_bilayer(z_dist=dist, a_shift=0.65, b_shift=0.65)
...
rows = zip(
    shift_arr, dist_arr, gap_arr, gap_soc_arr, gap_scissors_arr, gap_scissors_soc_arr
)

parprint(z_dist_arr)

with open(
    "gap_shift_lcao_3.25_constant_shift_0.65.csv", mode="w", newline=""
) as csvfile:
