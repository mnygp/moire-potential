...
for i in np.linspace(0, 1, 25):
    for j in np.linspace(0, 1, 25):
        bilayer = create_bilayer(z_dist=6.6, constrain=True, a_shift=i, b_shift=j)

        bilayer.calc = DFTD3(
            dft=GPAW(mode=PW(500), kpts=(8, 8, 1), xc="PBE", txt=None), xc="PBE"
        )

        uf = UnitCellFilter(bilayer, mask=[1, 1, 0, 0, 0, 1])
        relax = BFGS(
            uf,
            trajectory=f"traj_files/opt_{i:.2f}_{j:.2f}.traj",
