...
data = np.loadtxt("tree/write_csv_task/results.csv", delimiter=",", skiprows=1)
...
def scatter_plot(data, title, label, filename):
    ...
    #                     transform_coords[:, 0],
    scatter = ax.scatter(x_and_y[:, 0], x_and_y[:, 1], c=data, cmap="viridis")
...
scatter_plot(pre_gap - min(pre_gap), "Local gap pre relaxations", "Gap [eV]", "gap_pre")
scatter_plot(
    gap_no_strain - min(gap_no_strain),
...
scatter_plot(
    (gap_no_strain - min(gap_no_strain)) - (gap_with_strain - min(gap_with_strain)),
    "Difference in local gap: no cell opt - cell opt",
    "Gap [eV]",
    "gap_difference_no_cell_opt",
)

scatter_plot(
    dist_no_cell_opt - dist_cell_opt,
    "Difference in Z-distance",
    "Z-distance [Å]",
    "z_distance_difference",
)
scatter_plot(
    dist_ml - dist_cell_opt,
    "Z-distance: ML minus cell optimized relaxation",
    "Z-distance [Å]",
    "z_distance_ml",
)
scatter_plot(
    dist_ml - dist_no_cell_opt,
    "Z-distance: ML minus not cell optimized relaxation",
    "Z-distance [Å]",
    "z_distance_ml_no_cell_opt",
)
scatter_plot(
    dist_cell_opt,
