...
class single_cell:
    ...
    def create_struct_task(self):
        return tb.node("create_structure", i=self.x, j=self.y)

    @tb.task
    def pre_relax_gap_task(self):
        return tb.node("calc_gap", atom_path=self.create_struct_task, kpts=36)
    ...
    def post_relax_no_strain_gap_task(self):
        return tb.node("calc_gap", atom_path=self.relax_no_strain_task, kpts=36)
    ...
    def post_relax_with_strain_gap_task(self):
        return tb.node("calc_gap", atom_path=self.relax_with_strain_task, kpts=36)
...
def get_shifts(structure_path: str, root: str) -> dict[str, list[list[float]]]:
...
def return_as_dict(
    ...
    j: int,
    pre_relax: float,
    post_relax_no_strain: float,
    z_dist_no_strain: float,
    post_relax_with_strain: float,
    z_dist_with_strain: float,
    center: list[float],
    z_dist_ml,
...
def create_structure(i: float, j: float) -> Atoms:
    ...
    root = get_root_path("constrained-opt-MatterSim")
