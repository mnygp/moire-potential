...
def relax_structure(struct: Atoms, shift: float,
                    traj: str | None = None) -> Atoms:
...
for shift in shifts:
    ...
    for a in lattice_lengths:
        ...
        parprint(f'Shift: {shift:.3f}, Lattice length: {a:.3f} Å,'
                 + f' Gap before relaxation: {gap_before:.3f} eV')
        z_before = get_z(struct)

        relaxed_struct = relax_structure(struct, shift=shift,
                                         traj=f'traj_files/traj_{shift:.2f}_{a:.2f}.traj')
    ...
    with open(filename, mode='w', newline='') as csvfile:
        ...
        writer.writerow(['Lattice_length(Å)',
                         'Gap_before_relax(eV)',
                         'Z_distance_before',
                         'Gap_after_relax(eV)',
                         'Z_distance_after'])
        for a, gap_before, z_before, gap_after, z_after in results:
            writer.writerow([f'{a:.3f}',
                             f'{gap_before:.4f}',
                             f'{z_before:.4f}',
                             f'{gap_after:.4f}',
                             f'{z_after:.4f}'])
