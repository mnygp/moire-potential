...
path = "/structures/WSe2-WSe2/7.93_942/structure_ml.json"
...
class Workflow:
    ...
    def get_shifts_task(self):
        return tb.node(
            "get_shifts", structure_path=self.struct_path, root=self.get_root_task
        )

    @tb.dynamical_workflow_generator(
        {"results": "*/*", "gap_results": "*/return_dict_task"}
    )
    def generate_wfs(self):
        return tb.node("generate_wfs_task", input=self.get_shifts_task)  # type:ignore
