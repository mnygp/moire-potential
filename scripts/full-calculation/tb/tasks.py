...
def create_structure(z_dist: float,
                     a_shift: float,
                     b_shift: float) -> Path:
    struct = create_bilayer(z_dist,
    ...
    for parent in current_path.parents:
        if parent.name == root_dir_name:
            ...
            if csv_path.exists():
                ...
                raise FileNotFoundError(f"{filename} not found inside {root_dir_name} at {parent}")

    raise FileNotFoundError(f"Could not find a directory named '{root_dir_name}' from {current_path}")
