...
for i in range(len(x_WSe2)):
    ...
    indices = [atom.index for atom in bilayer if (atom.symbol == "W"
                                                  or atom.symbol == "Mo")]
...
data = np.genfromtxt("band_edges_medium_soc.csv", dtype=float,
...
with open("output.csv", "w", newline="") as f:
    writer = csv.writer(f)
    # write header
    writer.writerow(["x", "y", "distance_fixed_z", "distance"
                     "gap_fixed_z", "gap", "correction"])
    # write each row
    for i in range(len(x)):
        writer.writerow([x[i], y[i], inter_dist[i], new_z_dist[i],
                         gap_fixed_z[i], gap_arr[i], gap_correction_arr[i]])
